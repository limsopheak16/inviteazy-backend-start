"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.InviteController = void 0;
const uuid_1 = require("uuid");
class InviteController {
    constructor(inviteService, userService) {
        this.inviteService = inviteService;
        this.userService = userService;
    }
    getAllInvites(req, res, next) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const result = yield this.inviteService.getAllInvites();
                res.json({ message: "Get all invites.", data: result });
            }
            catch (error) {
                next(error);
            }
        });
    }
    getAllAcceptByenventID(req, res, next) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const { event_id } = req.params;
                const result = yield this.inviteService.getAllAcceptByenventID(event_id);
                res.json({ message: "Get all accepted invites.", data: result });
            }
            catch (error) {
                next(error);
            }
        });
    }
    updateInvitestatus(req, res, next) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const { id } = req.params;
                const { status } = req.body;
                if (!status) {
                    throw new Error("The 'status' field is required.");
                }
                const result = yield this.inviteService.updateInvitestatus(id, status);
                res.json({ message: "Invite status updated.", data: result });
                console.log('HHHHHHHHH', result);
            }
            catch (error) {
                next(error);
            }
        });
    }
    updateCheckinStatus(req, res, next) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const { id } = req.params;
                const existingInvite = yield this.inviteService.findById(id);
                if (!existingInvite) {
                    throw new Error("Invite not found");
                }
                if (existingInvite.is_checked_in === true) {
                    throw new Error("Invite is already checked in.");
                }
                const result = yield this.inviteService.updateCheckinStatus(id);
                res.json({ message: "Invite check-in updated.", data: result });
            }
            catch (error) {
                next(error);
            }
        });
    }
    updateCheckOutStatus(req, res, next) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const { id } = req.params;
                const { gift } = req.body;
                if (!gift) {
                    throw new Error("The 'gift' field is required.");
                }
                const existingInvite = yield this.inviteService.findById(id);
                if (!existingInvite) {
                    throw new Error("Invite not found");
                }
                if (existingInvite.is_checked_out) {
                    throw new Error("Invite is already checked out.");
                }
                if (!existingInvite.is_checked_in) {
                    throw new Error("Invite is not checked in.");
                }
                const inviteData = {
                    event_id: existingInvite.event_id,
                    user_id: existingInvite.user_id,
                    status: existingInvite.status,
                    qr_code: existingInvite.qr_code,
                    is_checked_in: existingInvite.is_checked_in,
                    check_in_time: existingInvite.check_in_time,
                    is_checked_out: true,
                    check_out_time: new Date(),
                    gift: gift,
                };
                const result = yield this.inviteService.updateCheckOutStatus(inviteData, id);
                res.json({ message: "Invite check-out updated.", data: result });
            }
            catch (error) {
                next(error);
            }
        });
    }
    findbyId(req, res, next) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const { id } = req.params;
                const result = yield this.inviteService.findById(id);
                res.json({ message: "Get invite by ID.", data: result });
            }
            catch (error) {
                next(error);
            }
        });
    }
    createInvite(req, res, next) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a;
            try {
                const { event_id } = req.params;
                const qr_code = (0, uuid_1.v4)();
                const result = yield this.userService.getUserById(req.userId);
                const userInvites = yield this.inviteService.findinvitebyuserID(req.userId);
                if (((_a = userInvites === null || userInvites === void 0 ? void 0 : userInvites.length) !== null && _a !== void 0 ? _a : 0) >= 50 && result.role !== "admin") {
                    throw new Error("You have reached the maximum limit of 50 invites.");
                }
                const newInvite = yield this.inviteService.createInvite({
                    event_id,
                    user_id: req.userId,
                    status: "pending",
                    qr_code,
                    is_checked_in: false,
                    check_in_time: null,
                    is_checked_out: false,
                    check_out_time: null,
                    gift: null,
                });
                res.status(201).json({
                    message: "A new invite was created.",
                    data: newInvite,
                    user: userInvites,
                    result,
                });
            }
            catch (error) {
                next(error);
            }
        });
    }
    getGuestInsight(req, res, next) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const { eventId } = req.params;
                const acceptedInvites = yield this.inviteService.getAllAcceptByenventID(eventId);
                const totalCheckedIn = yield this.inviteService.findAllcheckinByenventID(eventId);
                const totalContribution = yield this.inviteService.findAllmoneyByenventID(eventId);
                const totalAccepted = acceptedInvites ? acceptedInvites.length : 0;
                // const totalCheckedIn = acceptedInvites ? acceptedInvites.filter((invite) => invite.is_checked_in).length : 0;
                // const totalContribution = acceptedInvites
                //   ? acceptedInvites.reduce((sum, invite) => sum + (Number(invite.gift) || 0), 0)
                //   : 0;
                res.json({
                    message: "Guest Insight fetched successfully.",
                    data: {
                        totalAccepted,
                        totalCheckedIn,
                        totalContribution,
                    },
                });
            }
            catch (error) {
                next(error);
            }
        });
    }
}
exports.InviteController = InviteController;
